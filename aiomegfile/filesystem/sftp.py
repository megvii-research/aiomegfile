...
class AioSftpReadableFile(AioReadable[T.AnyStr], AioSeekable[T.AnyStr]):
    ...
    def _decode(self, data: bytes) -> T.AnyStr:
        """Decode bytes for text mode and passthrough bytes in binary mode."""
        if self._is_binary:
            return T.cast(T.AnyStr, data)
        return T.cast(T.AnyStr, data.decode(self._encoding, errors=self._errors))

    def _empty_value(self) -> T.AnyStr:
        """Return the empty value matching current mode."""
        if self._is_binary:
            return T.cast(T.AnyStr, b"")
        return T.cast(T.AnyStr, "")

    async def read(self, size: T.Optional[int] = None) -> T.AnyStr:
        """Read file content. ``size`` is interpreted in bytes."""
        self._ensure_open()

        if size == 0:
            return self._empty_value()

        read_size = -1 if size is None or int(size) < 0 else int(size)
        return self._decode(await self._read_raw(read_size))

    async def readline(self, size: T.Optional[int] = None) -> T.AnyStr:
        """Read one line from the file. ``size`` is interpreted in bytes."""
        self._ensure_open()
        ...
        while True:
            ...
            if limit is not None and len(line) + len(chunk) > limit:
                keep = limit - len(line)
                unread = len(chunk) - keep
                if unread > 0:
                    await self.seek(-unread, os.SEEK_CUR)
                chunk = chunk[:keep]

            newline_index = chunk.find(b"\n")
            if newline_index >= 0:
                keep = newline_index + 1
                unread = len(chunk) - keep
                if unread > 0:
                    await self.seek(-unread, os.SEEK_CUR)
                line.extend(chunk[:keep])
                break

            line.extend(chunk)

        return self._decode(bytes(line))

    async def seek(self, offset: int, whence: int = os.SEEK_SET) -> int:
        """Seek stream position in bytes."""
        self._ensure_open()
